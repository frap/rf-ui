(ns atea-ui.models.typeahead-test
  (:require [atea-ui.models.typeahead :as model]
            [atea-ui.specs.typeahead :as spec]
            [atea-ui.fake.server :as server]
...
(deftest querying-test
  (run-test-sync
   (stub-server-response "WTF" ["barme"])

   (let [t (re-frame/subscribe [::model/typeahead])]
     (testing "user types something aka WTF"
       (re-frame/dispatch [::model/on-query "WTF"])
  ...
  (run-test-sync
   ...
   (let [t (re-frame/subscribe [::model/typeahead])]
     (testing "user types something"
       ...
       (re-frame/dispatch [::model/on-query "Que?"])
